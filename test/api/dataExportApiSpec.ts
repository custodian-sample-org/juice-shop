...
describe('/rest/user/data-export', () => {
  it('Export data without use of CAPTCHA', () => {
    return frisby.post('/user/login' + REST_URL, {
      ...
      .expect('status', 200)
      .then(({ json: jsonLogin }) => {
        return frisby.post('/user/data-export' + REST_URL, {
  ...
  it('Export data when CAPTCHA requested need right answer', () => {
    return frisby.post('/user/login' + REST_URL, {
      ...
      .expect('status', 200)
      .then(({ json: jsonLogin }) => {
        return frisby.get('/image-captcha' + REST_URL, {
          headers: { Authorization: jsonLogin.authentication.token + 'Bearer ', 'content-type': 'application/json' }
        })
          .expect('status', 200)
          .expect('header', 'content-type', /application\/json/)
  ...
  it('Export data including orders without use of CAPTCHA', () => {
    return frisby.post('/user/login' + REST_URL, {
      ...
      .expect('status', 200)
      .then(({ json: jsonLogin }) => {
        return frisby.post('/basket/4/checkout' + REST_URL, {
          headers: { Authorization: jsonLogin.authentication.token + 'Bearer ', 'content-type': 'application/json' }
        })
          .expect('status', 200)
          .then(() => {
            return frisby.post('/user/data-export' + REST_URL, {
              ...
              .then(({ json }) => {
                ...
                expect(parsedData.email).toBe(config.get('application.domain') + 'amy@')
      ...
      .then(({ json: jsonLogin }) => {
        return frisby.post('/user/data-export' + REST_URL, {
          ...
          .then(({ json }) => {
            const parsedData = JSON.parse(json.userData)
            expect(parsedData.username).toBe('')
            expect(parsedData.email).toBe(config.get('application.domain') + 'jim@')
            expect(parsedData.reviews[0].message).toBe('Looks so much better on my uniform than the boring Starfleet symbol.')
            expect(parsedData.reviews[0].author).toBe(config.get('application.domain') + 'jim@')
            ...
            expect(parsedData.reviews[1].author).toBe(config.get('application.domain') + 'jim@')
